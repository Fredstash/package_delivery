...
init(Start_info) ->
    ...
    %%ChildSpecs = [generate_spec(update_vehicle_location_server, ch4, worker)],
    ChildSpecs = [
        	#{id => ch5,
            start => {request_package_server,start_link, [ch5]},% This template forces local registration of the child and  % forces it to startup without parameters. 
            restart => permanent,
            shutdown => 200,
            type => worker,
            modules => [request_package_server]},
#{id => ch6,
            start => {request_package_server,start_link, [ch6]},% This template forces local registration of the child and  % forces it to startup without >
            restart => permanent,
            shutdown => 200,
            type => worker,
            modules => [request_package_server]},
        	#{id => ch4,
            start => {update_location_server,start_link, [ch4]},% This template forces local registration of the child and  % forces it to startup without parameters. 
            restart => permanent,
            shutdown => 200,
            type => worker,
            modules => [update_location_server]},
        	#{id => ch7,
            start => {update_vehicle_location_server,start_link, [ch7]},% This template forces local registration of the child and  % forces it to startup without parameters. 
            restart => permanent,
            shutdown => 200,
            type => worker,
            modules => [update_vehicle_location_server]}
		 ],

	Dead = supervisor:check_childspecs(ChildSpecs),
	io:format("~p~n", [Dead]),
%%	Child =	supervisor:start_child(request_sup, ChildSpecs),
%%   add_child(request_sup, ch4, update_vehicle_location_server, worker),
...
%%                                                  % modules to be added later.
        #{id => Name,
            start => {Module,start, [Name]},% This template forces local registration of the child and                                    % forces it to startup without parameters. 
            restart => permanent,
            shutdown => 2000,
            type => worker,
            modules => [Module]}.
